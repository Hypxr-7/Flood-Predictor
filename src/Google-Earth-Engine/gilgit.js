// -----------------------------------------
// 1. ROI: Gilgit and Kashmir
// -----------------------------------------
var gilgit = countries
  .filter(ee.Filter.eq('ADM0_NAME', 'Jammu and Kashmir'))
  .geometry();

Map.centerObject(gilgit)
...
var meanLST = LST_Day.mean().clip(gilgit);
...
var meanNDVI = NDVI.mean().clip(gilgit);
...
var meanPrecipitation = precipitation.mean().clip(gilgit);
...
var meanNDSI = NDSI.mean().clip(gilgit);
