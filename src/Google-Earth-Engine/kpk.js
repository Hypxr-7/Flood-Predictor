...
var nwf = pakistan.filter(ee.Filter.eq('ADM1_NAME', 'North-West Frontier'));
var fata = pakistan.filter(ee.Filter.eq('ADM1_NAME', 'Federally Administered Tribal Areas'));


var combined = nwf.merge(fata);
var kpk = combined.geometry().dissolve(); // dissolve into single geometry

// Add to map
Map.centerObject(kpk);
Map.addLayer(kpk, { color: 'red' }, 'NWF + FATA');

// -----------------------------------------
// 2. Set Time Range
// -----------------------------------------
var startDate = ee.Date('2022-01-01');
var endDate = startDate.advance(3, 'year');
...
var meanLST = LST_Day.mean().clip(kpk);
...
var meanNDVI = NDVI.mean().clip(kpk);
...
var meanPrecipitation = precipitation.mean().clip(kpk);
...
var meanNDSI = NDSI.mean().clip(kpk);
